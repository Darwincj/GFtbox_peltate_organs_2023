...
%   Written at 2023-01-31 21:24:10.
    ...
    if (Steps(m)==0) && m.globalDynamicProps.doinit
        ...
        if m.globalProps.IFsetsoptions
            m = setUpModelOptions( m, ...
                'modelname', {'MODEL1',...  % Fig S14A, Simple leaf
                              'MODEL2',...  % Fig S14B, Unifacial leaf
                              'MODEL3',...  % Fig S14C, Peltate leaf
                              'MODEL4',...  % Fig S14D, Simple petal
                              'MODEL5',...  % Fig S14E, Peltately palmate compound leaf
                              'MODEL6',...  % Fig S14F, Pinnate compound leaf
...
%                          (m.FEnodes(:,3) - tip(3)).^2);
        ...
        switch OPTIONS.modelname 
            ...
            case 'MODEL1'
                ...
                id_sinus_p(:) = 0; 
                id_stalk_p(m.FEnodes(:,3) < 0.28) = 1;
                id_tip_p(RADII_TIP <= 0.025)=1;
                
            case 'MODEL2'
                
                id_blade_p((id_midplane_p)&(RADII_CENTER>0.95))=1;
                s_blade_p = id_blade_p;
                m.morphogenclamp((id_blade_p == 1), s_blade_i) = 1;
                m = leaf_mgen_conductivity( m, 'S_BLADE', 0.03 );
                m = leaf_mgen_absorption( m, 'S_BLADE', 0.001 );  
                
                id_sinus_p(:) = 0; 
                id_stalk_p(m.FEnodes(:,3) < 0.75) = 1;
                id_tip_p(RADII_TIP <= 0.025)=1;
    ...
    if (realtime > 4 - 0.001) && (realtime <= 20 - 0.001)   % time 18
        ...
        switch OPTIONS.modelname
            ...
            case 'MODEL2'
                ...
                kpar_p = 0.001;
                kper_p = 0.15 * (1 + 0.05*id_stalk_p + 0.5*id_blade_p);
                kpar2_p = 0.2 * (0.5+ 1*id_blade_p);
                
            case 'MODEL3'
                
                kpar_p = 0.001;
                kpar2_p = 0.25*pro(0.2,id_stalk_p).*(1 + 0.8*s_blade_p).*inh(3,id_top_p);
                kper_p = 0.12*inh(20,id_stalk_p).*(1 + 0.1*s_blade_p).*inh(3,id_top_p);
            ...
            case 'MODEL5'
                ...
                kpar2_p = 1 * pro(0.5,id_stalk_p) .* (0.2 + 0.6*s_blade_p)...
                           .* inh(3,id_top_p).*inh(20,id_sinus_p);
                kpar_p = 0.001;
                kper_p = 1 * inh(20,id_stalk_p) .* (0.1 + 0.4*s_blade_p)...
                          .* inh(3,id_top_p) .* inh(20,id_sinus_p);
